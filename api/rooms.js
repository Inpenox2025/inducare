...
module.exports = async function handler(req, res) {
  ...
  if (action === 'allocation') {
    ...
    if (req.method === 'POST') {
      try {
        ...
        const activePatientCheck = await sql`
          ...
          WHERE patient_id = ${parseInt(patient_id)} AND status = 'active'
        `;
        ...
        const roomCheck = await sql`SELECT status FROM rooms WHERE id = ${parseInt(room_id)}`;
        if (roomCheck.length === 0) return res.status(404).json({ error: 'Room not found' });
        ...
        const allocation = await sql`
          INSERT INTO room_allocations (room_id, patient_id, admitted_at, status)
          VALUES (${parseInt(room_id)}, ${parseInt(patient_id)}, NOW(), 'active')
          RETURNING *
        `;

        // Update room status to occupied
        await sql`UPDATE rooms SET status = 'occupied' WHERE id = ${parseInt(room_id)}`;
    ...
    if (req.method === 'PUT') {
      try {
        ...
        const rows = await sql`SELECT * FROM room_allocations WHERE id = ${parseInt(id)}`;
        if (rows.length === 0) return res.status(404).json({ error: 'Allocation not found' });
        const alloc = rows[0];
    ...
    if (req.method === 'POST') {
      try {
        ...
        const allocRows = await sql`
          ...
          FROM room_allocations ra
          JOIN patients p ON ra.patient_id = p.id
          WHERE ra.id = ${parseInt(allocation_id)}
        `;
        if (allocRows.length === 0) return res.status(404).json({ error: 'Active room allocation not found' });
        const allocation = allocRows[0];

        // Fetch doctor info
        const docRows = await sql`SELECT name, specialization FROM doctors WHERE id = ${parseInt(doctor_id)}`;
        ...
        const visit = await sql`
          INSERT INTO doctor_visits (allocation_id, doctor_id, visit_date, clinical_notes, temperature, blood_pressure, heart_rate, status)
          VALUES (${parseInt(allocation_id)}, ${parseInt(doctor_id)}, NOW(), ${clinical_notes || null}, ${temperature || null}, ${blood_pressure || null}, ${heart_rate || null}, 'completed')
          RETURNING *
        `;
        ...
        await sql`
          ...
          WHERE id = ${allocation.patient_id}
        `;
  ...
  // ══════════════════════════════════════════════════════════
    ...
    if (id) {
      if (req.method === 'GET') {
        try {
          const rows = await sql`SELECT * FROM rooms WHERE id = ${parseInt(id)}`;
    ...
    // Bulk operations
      ...
      if (req.method === 'GET') {
        try {
          ...
          if (query) {
            ...
            rows = await sql`
              SELECT * FROM rooms 
              WHERE room_no ILIKE ${searchPattern} 
                 OR room_type ILIKE ${searchPattern}
              ORDER BY room_no ASC
            `;
          } else {
            rows = await sql`SELECT * FROM rooms ORDER BY room_no ASC`;
          }
      ...
      if (req.method === 'POST') {
        try {
          ...
          // Verify room no unique
          const checkDup = await sql`SELECT id FROM rooms WHERE room_no = ${room_no.trim()}`;
          if (checkDup.length > 0) return res.status(400).json({ error: 'Room number already exists' });

          const rows = await sql`
            INSERT INTO rooms (room_no, room_type, status, price_per_day)
            VALUES (${room_no.trim()}, ${room_type ? room_type.trim() : null}, ${status || 'available'}, ${price_per_day || 0.00})
            RETURNING *
          `;

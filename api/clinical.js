...
module.exports = async function handler(req, res) {
  ...
  const sql = getSQL();
  const moduleType = req.query.module || (req.url && req.url.includes("/pharmacy") ? "pharmacy" : "lab");
  const action = req.query.action || (moduleType === "pharmacy" ? "medicines" : "tests");
  ...
  try {
    ...
    return res.status(404).json({ error: "Invalid action or module parameter" });
  } catch (error) {
    console.error(`[Clinical Handler Error] Module ${moduleType}, Action ${action}:`, error);
    return res.status(500).json({ error: "Clinical server request failed", details: error.message });
  }
};
